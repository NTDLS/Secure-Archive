...
HRESULT __stdcall CDataObject::GetData (FORMATETC *pFormatEtc, STGMEDIUM *pMedium)
{
	...
	//SendMessage(hStatusBar, (UINT)SB_SETTEXT, (WPARAM)1, (LPARAM)"OK!");
...
//
HRESULT __stdcall CDataObject::QueryGetData (FORMATETC *pFmtC)
{
	if((pFmtC->tymed &TYMED_HGLOBAL)
		&& pFmtC->cfFormat == CF_HDROP
		&& pFmtC->dwAspect == DVASPECT_CONTENT)
	{
		return (LookupFormatEtc(pFmtC) == -1) ? DV_E_FORMATETC : S_OK;
	}
	else{
		return DV_E_FORMATETC;
	}
